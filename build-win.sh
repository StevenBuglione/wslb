...
# Convert line endings for text files in distro/${distro}/config/docker
log "step" "Converting line endings in distro/${distro}/config/docker..."
find "distro/${distro}/config/docker" -type f \( \
    -name "*.sh" -o \
    -name "*.txt" -o \
    -name "*.conf" -o \
    -name "*.yml" -o \
    ...
    -name "*.Dockerfile" -o \
    -not -name "*.*" \
\) -exec sh -c 'echo "info: Converting: {}" && dos2unix "{}"' \;

# Convert line endings for text files in distro/${distro}/config/wsl
log "step" "Converting line endings in distro/${distro}/config/wsl..."
find "distro/${distro}/config/wsl" -type f \( \
    -name "*.sh" -o \
    -name "*.txt" -o \
    -name "*.conf" -o \
    -name "*.yml" -o \
...
cd "distro/${distro}" && docker build -t "${distro}-wsl" . || exit 1
cd - > /dev/null
