...
mkdir -p /usr/lib/wsl && cp /tmp/config/wsl/terminal-icon/arch.ico /usr/lib/wsl/arch.ico
...
# Arch Linux Requires We Configure locale
sed -i 's/^#\(en_US.UTF-8\)/\1/' /etc/locale.gen

# Generate the locales
locale-gen

# Set the LANG variable for the system
